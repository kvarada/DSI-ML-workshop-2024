...
The program learns based on the *features* present in the data, which represent the information we have about each example. If we are trying to predict the sale price of an apartment (for example), we might have a dataset with features (or variables) including square footage, number of bedrooms, and whether or not the apartment has an ocean view.
...
* For *classification* tasks, the target variable is categorical-- each example in the data belongs to one of a small number of categories. The goal is to get our model to correctly predict the category of an example based on its features.

* For *regression* tasks, the target variable is continuous. The model is required to predict the target with some degree of accuracy, so we minimize the numerical error in its predictions across the different examples.
...
## Unsupervised Learning
\
In unsupervised learning, there are no targets. The goal is instead to uncover patterns in the data. These can be used to provide a concise summary of the data, or group similar data together.
...
![](img/quiz2-grade-toy.png){fig-align="center"}

How would you go about predicting Quiz 2 grades from the given data?
...
## Overfitting-- Why we need a test set
\
...
Complex models are prone to overfitting-- we cannot just rely on training error to measure their performance.

It is necessary to set aside some data that is not shown to the model during training. This *test set* of data can then be used to evaluate whether the model predicts accurately on unseen data.

## The fundamental tradeoff in ML
\
The difference between training error and test error is called *generalization gap*. We want to minimize this gap, so that we can limit overfitting and ensure our model is learning reliable patterns in the data.

Simple models typically have a low generalization gap, so training error accurately approximates test error. But both training and test error will be high.

As we increase model complexity  we can usually reduce the training error and we hope this also reduces the test error. This may be true, however we will typically introduce a larger generalization gap as our model starts to overfit.

## 

Thus we have the "fundamental tradeoff": as we increase model complexity, the training error will reduce but the gap between training and test error will increase.
